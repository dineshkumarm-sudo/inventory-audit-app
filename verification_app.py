...
st.write("Upload your product export spreadsheet to run the metadata and round-count audit engine.")

# File Uploader component
uploaded_file = st.file_uploader("Choose your Excel file (.xlsx)", type=["xlsx"])

if uploaded_file is not None:
    # 1. Read input spreadsheet layout structure
    df = pd.read_excel(uploaded_file)
    st.success(f"Successfully loaded {len(df)} rows!")

    # Dynamic column mapping choices for the user
    ...
    if st.button("🚀 Run Complete Dual-Engine Audit"):
        ...
        # Clean up existing core columns to prevent string formatting mismatch anomalies
        for col in ['MPN', 'UPC', 'Brand']:
            if col in df.columns:
                df[col] = df[col].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
                df[col] = df[col].replace('nan', '')

        # Insert placeholder columns directly after the Description column
        desc_position = df.columns.get_loc(desc_col)

        # Avoid duplicate column errors if processed multiple times
        new_cols = ['Extracted Brand', 'Extracted UPC', 'Extracted MPN', 'Extracted Title Rounds', 
        ...
        df.insert(desc_position + 4, 'Extracted Title Rounds', None)
        df.insert(desc_position + 5, 'Extracted Desc Quantity', None)
        df.insert(desc_position + 6, 'Extracted Packaging Rounds', None)
        df.insert(desc_position + 7, 'Metadata Verification Comment', None)
        df.insert(desc_position + 8, 'Rounds Verification Comment', None)

        # Write to byte buffer for openpyxl processing in-memory
        ...
        # Save output layout out back to a brand new binary data stream buffer
        out_buffer = io.BytesIO()
        wb.save(out_buffer)
        out_buffer.seek(0)

        # Download Trigger interface element
        st.download_button(
